...
class form_auction extends XoopsThemeForm
{
	...
	function form_auction(&$AuctionObj,&$RoomList,&$HotelList)
	{
		$this->Obj = & $AuctionObj;
		$this->RoomList = & $RoomList;
		$this->HotelList = & $HotelList;
		$this->XoopsThemeForm( '竞价', "op", xoops_getenv('PHP_SELF') . "?action=save" );
	...
	function createElements()
	{
		...
		//$this->addElement( new XoopsFormTextDateSelect("开始竞价时间", 'apply_start_date', $size = 15, $this->Obj->apply_start_date(),false ) ,true);
		$this->addElement( new MartinFormDateTime("开始竞价时间", 'apply_start_date', $size = 15, $this->Obj->apply_start_date() ) ,true);
		//$this->addElement( new XoopsFormTextDateSelect("结束竞价时间", 'apply_end_date', $size = 15, $this->Obj->apply_end_date() ) ,true);
		$this->addElement( new MartinFormDateTime("结束竞价时间", 'apply_end_date', $size = 15, $this->Obj->apply_end_date() ) ,true);
		
		$this->addElement( new XoopsFormText('起拍价', 'auction_price', 11, 11, $this->Obj->auction_price()), true);
		$this->addElement( new XoopsFormText('低价', 'auction_low_price', 11, 11, $this->Obj->auction_low_price()), true);
		$this->addElement( new XoopsFormText('价格(加价幅度)', 'auction_add_price', 11, 11, $this->Obj->auction_add_price()), true);
		$this->addElement( new XoopsFormText('赠送现金卷?', 'auction_sented_coupon', 11, 11, intval($this->Obj->auction_sented_coupon())), true);
		$this->addElement( new XoopsFormRadioYN("能否使用现金卷", 'auction_can_use_coupon', $this->Obj->auction_can_use_coupon(), '能', '不能') , true);
		$this->addElement( new XoopsFormRadioYN("状态", 'auction_status', $this->Obj->auction_status(), '已发布', '编辑中') , true);
		
		$this->addElement( new XoopsFormText('标题', 'auction_name', 50, 255, $this->Obj->auction_name()), true);
		$editor = 'tinymce';
		...
		$this->addElement(new XoopsFormEditor("竞价详细信息", $editor, $editor_configs, false, $onfailure = null) , false);
