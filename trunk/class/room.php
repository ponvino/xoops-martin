<?php
/**
* $Id: room.php,v 1.42 2007/02/04 15:01:40 malanciault Exp $
* Module:martin 
* Licence: GNU
*/
...
class MartinRoom extends XoopsObject
{
	...
	function room_bed_info($format = 'S')
	{
		return $this->getVar("room_bed_info",$format);
	}

	function room_sented_coupon($format = 'S')
	{
...
class MartinRoomHandler extends XoopsObjectHandler
{
	...
	*/
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function &getRooms($limit=0, $start=0, $sort='room_id', $order='ASC', $id_as_key = true)
	{
	...
	*
	* @param object $room reference to the {@link room} object
	* @param bool $force
	* @return bool FALSE if failed, TRUE if already present and unchanged or successful
	*/
	function insert(&$room, $force = false)
	{

		if (strtolower(get_class($room)) != 'martinroom') {
			return false;
		}
		...
		if ($room->isNew()) {
			$sql = sprintf("INSERT INTO %s (
								...
								$room_type_id,$hotel_id,$room_count,$room_bed_type,
								$this->db->quoteString($room_name),
								$room_area,
								$this->db->quoteString($room_floor),
								$room_initial_price,
								$room_is_add_bed,$room_add_money,
								$this->db->quoteString($room_bed_info),
								$room_status,$room_sented_coupon
								);
		} else {
	...
	public function CheckHotelRoomExist($roomObj)
	{
		...
		if(($roomObj->isNew() && $count > 0) || $count > 1) return true;
		return false;
	}
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function delete(&$room, $force = false)
	{
	...
	*/
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function getRoomTypeList($room_type_id = 0)
	{
		$rows = array();
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function getRoomList($room_id = 0)
	{
		$rows = array();
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function insertType($typeData)
	{
		global $xoopsDB;
		if(empty($typeData)) return $typeData;
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function getRoomPrice($room_id = 0,$room_date = 0)
	{
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function GetRoomPriceList($limit,$start = 0)
	{
		$rows = array();
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function TruncatePassData($date = 0)
	{
		global $xoopsDB;
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function deleteRoomPrice($room_id,$room_date)
	{
		global $xoopsDB;
		if(!$room_id || !$room_date) return false;
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function InsertRoomPrice($Data,$IsOld)
	{
		//var_dump($Data);exit;
		global $xoopsDB;
		if($IsOld)
		{
			$sql = "UPDATE ".$xoopsDB->prefix("martin_room_price")." set 
				...
				room_is_totay_special = {$Data['room_is_totay_special']},
			...
			$sql = "insert INTO ".$xoopsDB->prefix("martin_room_price")." (room_id,room_is_totay_special,room_price,room_advisory_range_small,room_advisory_range_max,room_sented_coupon,room_date ) VALUES ";
			$Insert = false;
			foreach($Data as $price)
			{
				if(!$this->CheckExistDate($price['room_id'],$price['room_date'])){
					$sql .= $prefix . "({$price['room_id']},{$price['room_is_totay_special']},{$price['room_price']},
					...
					$upSql = "UPDATE ".$xoopsDB->prefix('martin_room_price')." SET 
							...
							room_is_totay_special = {$price['room_is_totay_special']}
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function CheckExistDate($room_id , $room_date)
	{
		global $xoopsDB;
	 ...
	 * @author Martin <china.codehome@gmail.com> 
	 * */
	function GetRoomDatePrie($room_id,$check_in_date,$check_out_date)
	{
		global $xoopsDB;
		if(!$room_id || !$check_in_date || !$check_out_date) return false;
