...
function uploadSuccess(file, serverData) {
	try {
		...
		if (serverData.substring(0, 7) === "FILEID:") {
			addImage("thumbnail.php?id=" + serverData.substring(7),this.customSettings.showUpload);
			progress.setComplete();
			progress.setStatus("Upload Complete.");
			progress.toggleCancel(false);
		} else {
			addImage("../javascript/swfupload/images/error.gif",this.customSettings.showUpload);
...
function addImage(src,ShowUpload) {
	var newImg = document.createElement("img");
	newImg.style.margin = "5px";
	document.getElementById(ShowUpload).appendChild(newImg);
	if (newImg.filters) {
		try {
